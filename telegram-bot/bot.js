...
// Для завсклада — данные только по своей группе складов, для остальных — все склады
const getDataForUser = async (userId) => {
    const s = getSession(userId);
    if (s.role === 'warehouse') {
        const rawData = await getDataAll(userId);
        return filterByWarehouseGroup(rawData, userId);
    }
    return getDataAll(userId);
};
...
bot.action('excoalition', async (ctx) => {
    ...
    try {
        ...
        ws.getRow(2).values = ['', 'тонна', 'цена', 'сумма', 'остаток'];
        ...
        let rowNum = 3;
        let grandTonsAcc = 0, grandSumAcc = 0;
